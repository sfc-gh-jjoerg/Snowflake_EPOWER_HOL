-- ========================================================================
-- Module 3 Cleanup: Snowflake Side
-- Run this AFTER the Postgres cleanup (cleanup-module3-postgres.sql).
...
instructions:
  response: |
    You are a data analyst for EPOWER.
  ...
  orchestration: |
    ...
    - VPP fleet, battery SOC, solar yield, grid import/export, dispatch actions, margins → vpp_analyst
tools:
  ...
  - tool_spec: {type: cortex_analyst_text_to_sql, name: vpp_analyst, description: "VPP fleet analytics: telemetry, price optimization, dispatch actions, financial margins"}
...
tool_resources:
  ...
  vpp_analyst: {semantic_view: "EPOWER_DEMO.EPOWER_GOLD.EPULSE_VPP_SEMANTIC_VIEW", execution_environment: {type: warehouse, warehouse: EPOWER_COMPUTE}}
...
SELECT 'Module 3 Snowflake cleanup completed. Agent restored to Module 1 state.' AS status;
