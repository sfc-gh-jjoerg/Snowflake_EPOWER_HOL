...
2. **View design** — Proposed creating 4 pre-aggregated SQL views (`V_VPP_MONITOR_KPI`, `V_VPP_MONITOR_TIMESERIES`, `V_VPP_MONITOR_ACTIONS`, `V_VPP_MONITOR_MAP`) to keep dashboard queries fast, plus a `CITY_CLUSTER_MAP` table to join cities to VPP clusters
...
| **Backend views** | `sql/create_views.sql` — 4 pre-aggregated views + city-cluster mapping table |
